...
const DoctorDetails = () => {
  ...
  const fetchDoctorDetails = async (id) => {
    try {
      console.log(`Fetching details for doctor with id: ${id}`);
      const response = await axios.post(`${PROD_URL}/doctor`, {
        data: {
          action: "getSingleDoctor",
          id: id,
        },
      });

      if (response.data.statusCode === 200) {
        const { doctor } = response.data.body;
        console.log(`Doctor_Details: ${JSON.stringify(doctor)}`);
        setDoctor(doctor);
        setLoading(false);
        setError(false);
      }
  ...
  useEffect(() => {
    console.log(`USE_EFFECT: Start fetching doctor details...`);
